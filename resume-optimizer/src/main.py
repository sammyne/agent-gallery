...
def main():
    ...
    with st.sidebar:
        ...
        generative_model = st.selectbox(
            "Generative Model", ["qwen3-max-2025-09-23", "deepseek-v3.2-exp"], index=0
        )

        # 向量化模型选择
        embedding_model = st.selectbox(
            "向量化模型", ["text-embedding-v4", "text-embedding-v2"], index=0
        )
        ...
        uploaded_file = st.file_uploader(
            "Choose your resume (PDF)", type="pdf", accept_multiple_files=False
        )
